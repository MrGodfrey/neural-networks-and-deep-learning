...
# ── 练习 1 测试 1：两层网络初始化（n_x=3, n_h=2, n_y=1）──────────────────────
def initialize_parameters_test_1(target):
    ...
    expected_output = {"W1": expected_W1, "b1": expected_b1,
                       "W2": expected_W2, "b2": expected_b2}
    test_cases = [
        {"name": "datatype_check",  "input": [n_x, n_h, n_y],
         "expected": expected_output, "error": "数据类型不匹配"},
        {"name": "equation_output_check", "input": [n_x, n_h, n_y],
         "expected": expected_output, "error": "输出结果有误"},
    ]
    multiple_test(test_cases, target)


# ── 练习 1 测试 2：两层网络初始化（n_x=4, n_h=3, n_y=2）──────────────────────
...
def initialize_parameters_deep_test_1(target):
    ...
    expected_output = {"W1": expected_W1, "b1": expected_b1,
                       "W2": expected_W2, "b2": expected_b2}
    test_cases = [
        {"name": "datatype_check",      "input": [layer_dims],
         "expected": expected_output, "error": "数据类型不匹配"},
        {"name": "shape_check",         "input": [layer_dims],
         "expected": expected_output, "error": "形状有误"},
        {"name": "equation_output_check", "input": [layer_dims],
         "expected": expected_output, "error": "输出结果有误"},
    ]
    multiple_test(test_cases, target)


# ── 练习 2 测试 2：L 层网络初始化（[4,3,2]）──────────────────────────────────
...
def linear_forward_test(target):
    ...
    test_cases = [
        {"name": "datatype_check",      "input": [A_prev, W, b],
         "expected": expected_output, "error": "数据类型不匹配"},
        {"name": "shape_check",         "input": [A_prev, W, b],
         "expected": expected_output, "error": "形状有误"},
        {"name": "equation_output_check", "input": [A_prev, W, b],
         "expected": expected_output, "error": "输出结果有误"},
    ]
    multiple_test(test_cases, target)
...
def linear_activation_forward_test(target):
    ...
    test_cases = [
        {"name": "datatype_check",      "input": [A_prev, W, b, "sigmoid"],
         "expected": expected_output_sigmoid, "error": "sigmoid 激活：数据类型不匹配"},
        {"name": "shape_check",         "input": [A_prev, W, b, "sigmoid"],
         "expected": expected_output_sigmoid, "error": "sigmoid 激活：形状有误"},
        {"name": "equation_output_check", "input": [A_prev, W, b, "sigmoid"],
         "expected": expected_output_sigmoid, "error": "sigmoid 激活：输出结果有误"},
        {"name": "datatype_check",      "input": [A_prev, W, b, "relu"],
         "expected": expected_output_relu, "error": "relu 激活：数据类型不匹配"},
        {"name": "shape_check",         "input": [A_prev, W, b, "relu"],
         "expected": expected_output_relu, "error": "relu 激活：形状有误"},
        {"name": "equation_output_check", "input": [A_prev, W, b, "relu"],
         "expected": expected_output_relu, "error": "relu 激活：输出结果有误"},
    ]
    multiple_test(test_cases, target)
...
def L_model_forward_test(target):
    ...
    test_cases = [
        {"name": "datatype_check",      "input": [X, parameters],
         "expected": expected_output, "error": "应返回 numpy 数组"},
        {"name": "shape_check",         "input": [X, parameters],
         "expected": expected_output, "error": "形状有误"},
        {"name": "equation_output_check", "input": [X, parameters],
         "expected": expected_output, "error": "输出结果有误"},
    ]
    multiple_test(test_cases, target)
...
def compute_cost_test(target):
    ...
    test_cases = [
        {"name": "equation_output_check", "input": [AL, Y],
         "expected": expected_output, "error": "输出结果有误"},
    ]
...
def linear_backward_test(target):
    ...
    test_cases = [
        {"name": "datatype_check",      "input": [dZ, linear_cache],
         "expected": expected_output, "error": "数据类型不匹配"},
        {"name": "shape_check",         "input": [dZ, linear_cache],
         "expected": expected_output, "error": "形状有误"},
        {"name": "equation_output_check", "input": [dZ, linear_cache],
         "expected": expected_output, "error": "输出结果有误"},
    ]
    multiple_test(test_cases, target)
...
def linear_activation_backward_test(target):
    ...
    test_cases = [
        {"name": "datatype_check",      "input": [dA, linear_activation_cache, "sigmoid"],
         "expected": expected_output_sigmoid, "error": "sigmoid：数据类型不匹配"},
        {"name": "shape_check",         "input": [dA, linear_activation_cache, "sigmoid"],
         "expected": expected_output_sigmoid, "error": "sigmoid：形状有误"},
        {"name": "equation_output_check", "input": [dA, linear_activation_cache, "sigmoid"],
         "expected": expected_output_sigmoid, "error": "sigmoid：输出结果有误"},
        {"name": "datatype_check",      "input": [dA, linear_activation_cache, "relu"],
         "expected": expected_output_relu, "error": "relu：数据类型不匹配"},
        {"name": "shape_check",         "input": [dA, linear_activation_cache, "relu"],
         "expected": expected_output_relu, "error": "relu：形状有误"},
        {"name": "equation_output_check", "input": [dA, linear_activation_cache, "relu"],
         "expected": expected_output_relu, "error": "relu：输出结果有误"},
    ]
    multiple_test(test_cases, target)
...
def L_model_backward_test(target):
    ...
    A0 = np.random.randn(4, 2);  W1 = np.random.randn(3, 4)
    b1 = np.random.randn(3, 1);  Z1 = np.random.randn(3, 2)
    cache_layer_1 = ((A0, W1, b1), Z1)

    A1 = np.random.randn(3, 2);  W2 = np.random.randn(1, 3)
    b2 = np.random.randn(1, 1);  Z2 = np.random.randn(1, 2)
    ...
    expected_output = {"dA1": expected_dA1, "dW2": expected_dW2, "db2": expected_db2,
                       "dA0": expected_dA0, "dW1": expected_dW1, "db1": expected_db1}
    test_cases = [
        {"name": "datatype_check",      "input": [AL, Y, caches],
         "expected": expected_output, "error": "数据类型不匹配"},
        {"name": "shape_check",         "input": [AL, Y, caches],
         "expected": expected_output, "error": "形状有误"},
        {"name": "equation_output_check", "input": [AL, Y, caches],
         "expected": expected_output, "error": "输出结果有误"},
    ]
    multiple_test(test_cases, target)
...
def update_parameters_test(target):
    ...
    expected_output = {"W1": expected_W1, "b1": expected_b1,
                       "W2": expected_W2, "b2": expected_b2}
    test_cases = [
        {"name": "datatype_check",      "input": [parameters, grads, learning_rate],
         "expected": expected_output, "error": "数据类型不匹配"},
        {"name": "shape_check",         "input": [parameters, grads, learning_rate],
         "expected": expected_output, "error": "形状有误"},
        {"name": "equation_output_check", "input": [parameters, grads, 0.1],
         "expected": expected_output, "error": "输出结果有误"},
    ]
    multiple_test(test_cases, target)
