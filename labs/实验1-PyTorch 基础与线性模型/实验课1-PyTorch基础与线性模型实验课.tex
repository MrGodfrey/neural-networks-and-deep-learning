...
对于学有余力且富有创造力的同学, 除上述基础实验外, 还可以完成以下补充选做内容, 用于探索更完整的工程实践与更开放的创意应用. \emph{请务必仔细阅读最后一节关于作业的重要说明.}
...
\begin{enumerate}
    ...
    \item \textbf{选做}: 补充和完善课程知识文档或环境配置说明. 例如, 在现有 Mac 配置说明的基础上, 增加 Windows 环境下的安装与运行步骤, 或补充校园网、代理网络、镜像源等特殊网络环境中的配置方法; 鼓励将整理后的 Markdown 文档提交到课程代码仓库, 共同完善课程资料.
    \item \textbf{选做}: 基于 PyTorch 与线性模型实现一个有意思的小项目或创意应用. 你可以尝试用线性回归、Logistic 回归、Softmax 回归、感知器或 SVM 解决一个自选问题, 也可以在基础框架上探索更有想象力的应用形式; 提交时需同时说明你的任务设定、实现思路、实验结果以及创意亮点.
\end{enumerate}
...
请在提交作业前逐项检查材料是否齐全. 本次实验提交内容分为两部分: 一部分是各实验对应的 Jupyter Notebook 文件, 另一部分是记录你与 AI 工具互动过程的文本文件. 本课程作业上传只接受文本文件, 不接受 PDF、Word 文档、图片、截图、压缩包等二进制文件. 采用这一规则, 一方面是为了引入 AI 辅助评阅, 另一方面也是希望大家养成用清晰文本表达实验过程和结论的习惯.
...
\begin{enumerate}
    ...
    \begin{enumerate}
        \item 必交内容包括实验1、实验2、实验3对应的 Jupyter Notebook 文件, 文件格式统一为 \texttt{.ipynb}.
        \item 若完成了选做题, 也请提交对应的 Jupyter Notebook 文件, 格式同样统一为 \texttt{.ipynb}.
        \item 提交时只需提交 Notebook 源文件, 不要额外提交同内容的 \texttt{.pdf}、\texttt{.html}、\texttt{.py}、Word 文档、截图或图片文件.
        \item 文件命名请尽量直接、清晰, 按照内容分别命名为: \texttt{实验1}、\texttt{实验2}、\texttt{实验3}; 若提交选做题, 则命名为 \texttt{选择1}、\texttt{选择2}、\texttt{选择3}、\texttt{选择4}. 实际提交文件时, 请保留完整扩展名, 例如 \texttt{实验1.ipynb}.
        \item 请确保每个 Notebook 中包含你实际完成的代码、运行结果与必要的实验记录, 避免出现文件名正确但内容缺失、未保存或与题目不对应的情况.
    \end{enumerate}
    \item \textbf{实验过程与 AI 互动记录文本文件(.md / .txt)}
    \begin{enumerate}
        \item 必须额外提交一份文本文件, 作为``询问与 AI 互动的过程记录'', 推荐使用 \texttt{.md} 或 \texttt{.txt}.
        \item 这份记录中, 你至少需要完整记录自己在完成实验过程中向 AI 提出的所有问题.
        \item 不要把 AI 的回复原文放入记录中, 也不要复制粘贴大段代码; 记录应只包含你自己写的内容, 包括你的问题、判断、尝试、修改思路和必要的简短总结.
        \item 记录时不要只保留少量片段或最后一次提问, 而应尽可能按时间顺序整理完整, 使教师能够看清你在实验过程中如何思考、如何求助以及如何逐步完成任务.
        \item 若同一个问题你追问了多次, 或围绕同一处代码反复向 AI 询问, 也应如实记录, 不要合并省略到只剩一句概括.
        \item 建议将该文件命名得清楚易辨认, 例如 \texttt{AI问答记录.md} 或 \texttt{实验过程记录.md}, 以免与 Notebook 文件混淆.
    \end{enumerate}
\end{enumerate}
...
若希望获得较高分数, 必须在实验操作过程中如实记录自己的思考、尝试与求助过程, 并整理为 Markdown 或纯文本文件随作业一并提交. 记录内容至少应包括:

\begin{enumerate}
    \item 你查阅了哪些资料.
    \item 你向 AI 提出了哪些问题.
    \item 你如何根据这些交流继续思考、调试、修改代码或完善实验分析.
\end{enumerate}

记录文件应只保留你自己写的内容. 不要粘贴 AI 的完整回复, 不要粘贴复制来的大段代码, 也不要把聊天记录原样导出后提交. 记录得越清楚, 就越能证明该实验过程主要由你独立完成, 而不是直接依赖工具生成最终答案.

在初学阶段, 除选做题外, 本实验中存在的代码空缺不得直接使用 AI 工具自动补全, 你必须自己手写实现. 你可以向 AI 询问``下一步应使用什么工具''或``某个 API 的作用是什么'', 但相关提问、你的理解和你的后续修改过程同样需要如实记录在文本文件中, 作为学习过程的证明.
