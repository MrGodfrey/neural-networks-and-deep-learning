...
def load_happy_dataset():
    ...
    返回值：
        X_train -- 训练集图像，形状 (m_train, H, W, C)
...
def random_mini_batches(X, Y, mini_batch_size=64, seed=0):
    """
    将数据集 (X, Y) 随机打乱并划分为若干 mini-batch。

    参数：
        X              -- 输入数据，形状 (m, H, W, C)
        Y              -- 标签，形状 (m, n_y)
        mini_batch_size -- 每个 mini-batch 的样本数，默认 64
        seed           -- 随机种子，用于复现结果
    ...
    # 步骤 2：按 mini_batch_size 切分完整的 mini-batch
    ...
    # 步骤 3：处理最后一个不足 mini_batch_size 的尾部批次
