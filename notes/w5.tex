...
除本导读外, 本讲共有 5 个主体节. 它们从卷积神经网络为什么适合图像开始, 先讲卷积运算本身, 再讲卷积层、池化层和反向传播, 最后介绍典型 CNN 结构及其应用.

\begin{enumerate}
    \item \textbf{卷积神经网络概览}: 说明全连接网络直接处理图像的问题, 以及 CNN 如何利用局部连接、权重共享和下采样.
    \item \textbf{卷积的定义}: 说明一维和二维卷积的数学形式、步长、填充、输出尺寸以及滤波器视角.
    \item \textbf{卷积神经网络}: 说明多通道卷积、多个卷积核、池化、卷积块、感受野、空洞卷积和转置卷积如何组成 CNN 模型.
    \item \textbf{卷积神经网络的例子}: 通过 LeNet-5、AlexNet、Inception 和 ResNet 理解卷积网络结构的发展脉络.
    \item \textbf{卷积神经网络的应用}: 说明 CNN 如何用于棋盘决策、目标检测、实例分割、OCR、图像生成和鲁棒性分析等任务.
\end{enumerate}

\textbf{如果细节都忘了}: 最应该记住的是, CNN 把图像等结构化数据中的“局部模式会重复出现”写进模型结构: 局部连接负责看小区域, 权重共享负责复用同一模式检测器, 多层堆叠负责把边缘和纹理组合成更高层语义.

\textbf{问题思考}:
\begin{itemize}
    \item 为什么同样是前馈神经网络, CNN 在图像任务上通常比直接全连接更合适?
\end{itemize}
...
\textbf{本节导读}:
\begin{itemize}
    \item \textbf{本节目的}: 说明为什么图像等空间结构数据需要卷积网络, 而不是简单展平后接全连接层.
    \item \textbf{为什么学}: CNN 的优势来自结构先验, 只有先理解局部连接、权重共享和下采样, 后面卷积公式和网络结构才有意义.
    \item \textbf{核心内容}: CNN 用局部感受野保留邻接关系, 用参数共享减少参数量并利用空间重复性, 用下采样提高计算效率和局部鲁棒性.
    \item \textbf{最小例子}: 一幅 $1000\times1000$ RGB 图像若直接连到 $1000$ 个隐藏单元, 参数量约为 $3\times10^9$; 若用 $3\times3$ 卷积核, 参数量只与核大小和通道数有关, 不随整幅图像面积成倍膨胀.
    \item \textbf{最该记住}: CNN 的核心不是“多了一种层”, 而是把图像中的局部性和位置复用先验写进了网络结构.
\end{itemize}
...
\textbf{本节导读}:
\begin{itemize}
    \item \textbf{本节目的}: 从信号处理和数学公式角度说明卷积到底是什么, 以及它如何用于局部特征提取.
    \item \textbf{为什么学}: 不理解卷积、互相关、步长和填充, 就很难正确设计 CNN 的输入输出尺寸和解释卷积核作用.
    \item \textbf{核心内容}: 卷积是局部加权叠加, 可看作滤波器在输入上滑动; 步长控制采样间隔, 填充控制边界处理, 卷积核决定局部模式.
    \item \textbf{最小例子}: 一维滤波器 $[1,\frac{1}{2},\frac{1}{4}]$ 作用在序列上时, 当前输出等于当前输入和过去两个时刻输入的加权和, 这就是延迟信息的局部叠加.
    \item \textbf{最该记住}: 卷积的本质是用同一个局部滤波器在不同位置重复检测相同类型的模式.
\end{itemize}
...
\textbf{本节导读}:
\begin{itemize}
    \item \textbf{本节目的}: 把卷积运算放进完整神经网络中, 说明卷积层、通道、池化和感受野如何协同工作.
    \item \textbf{为什么学}: 单个卷积公式还不是 CNN, 真正的 CNN 需要多通道特征、非线性激活、下采样和分类头共同组成层级模型.
    \item \textbf{核心内容}: 多个卷积核产生多个输出通道, 池化压缩空间尺寸, 多层堆叠扩大感受野并形成从局部到语义的层级表示.
    \item \textbf{最小例子}: 一个 RGB 输入有 3 个通道, 若某层使用 16 个卷积核, 每个输出通道都要综合 3 个输入通道的局部信息, 最终得到 16 张特征图.
    \item \textbf{最该记住}: 卷积层主要生成新特征, 池化或步长卷积主要压缩空间尺寸, 感受野则描述高层特征能看到多大输入区域.
\end{itemize}
...
\textbf{本节导读}:
\begin{itemize}
    \item \textbf{本节目的}: 通过典型 CNN 结构理解卷积网络从早期手写数字识别到深层残差网络的发展.
    \item \textbf{为什么学}: 经典结构展示了不同设计思想如何解决规模、计算、多尺度特征和深层优化问题.
    \item \textbf{核心内容}: LeNet-5 展示卷积和下采样的早期范式, AlexNet 证明深层 CNN 在大规模视觉任务中的威力, Inception 使用多尺度并行分支, ResNet 用残差连接训练更深网络.
    \item \textbf{最小例子}: ResNet 的一个残差块写成 $\boldsymbol{y}=\mathcal{F}(\boldsymbol{x})+\boldsymbol{x}$, 若最优变换接近恒等映射, 只需让 $\mathcal{F}(\boldsymbol{x})$ 接近 0 即可.
    \item \textbf{最该记住}: CNN 结构演化的主线是更有效地提取多尺度特征, 同时让更深的网络仍然可训练.
\end{itemize}
...
\textbf{本节导读}:
\begin{itemize}
    \item \textbf{本节目的}: 说明卷积网络不仅用于图像分类, 也能作为许多空间结构任务的基础表征模块.
    \item \textbf{为什么学}: 应用场景能帮助理解 CNN 学到的不是固定任务答案, 而是可迁移的局部和层级视觉表示.
    \item \textbf{核心内容}: CNN 可用于棋盘状态建模、检测、分割、OCR、特征可视化、风格迁移、图像生成和对抗鲁棒性分析.
    \item \textbf{最小例子}: 在围棋中, $19\times19$ 棋盘也可以看作多通道二维网格, 卷积核能检测局部棋形和连接关系, 因而 CNN 可服务于策略网络和价值网络.
    \item \textbf{最该记住}: 只要数据具有局部结构和模式复用, 卷积网络就可能成为有效的表征学习工具.
\end{itemize}
