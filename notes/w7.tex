...
\begin{enumerate}
  ...
  \item \textbf{batch size 与线性缩放规则}
    \begin{itemize}
      ...
      \item 线性缩放规则给出一个常用近似: 若 batch size 从 $K$ 增大到 $cK$, 学习率可从 $\alpha$ 增大到 $c\alpha$. 下面用一个 epoch 内的参数位移说明这个规则. 设训练集共有 $N=MK$ 个样本, 并假设在这个很短的比较区间内, 每个样本给出的梯度都近似为同一个向量 $\boldsymbol{g}$. 当 batch size 为 $K$、学习率为 $\alpha$ 时, 一个 mini-batch 梯度为
      \[
        \frac{1}{K}\sum_{i=1}^{K}\boldsymbol{g}
        \approx
        \boldsymbol{g}.
      \]
      因为一个 epoch 要把 $MK$ 个样本大致用完, 所以需要 $M$ 次迭代. 若每一步的梯度平均值都近似为 $\boldsymbol{g}$, 一个 epoch 后的参数位移约为
      \[
        \Delta\boldsymbol{\theta}_{\mathrm{small}}
        \approx
        -\alpha\sum_{j=1}^{M}\boldsymbol{g}
        =
        -M\alpha\boldsymbol{g}.
      \]
      若把 batch size 直接增大到 $MK$, 一个 epoch 只需一次迭代. 此时全量 batch 的平均梯度为
      \[
        \frac{1}{MK}\sum_{i=1}^{MK}\boldsymbol{g}
        \approx
        \boldsymbol{g}.
      \]
      记对应学习率为 $\tilde{\alpha}$, 则一个 epoch 的参数位移约为
      \[
        \Delta\boldsymbol{\theta}_{\mathrm{large}}
        \approx
        -\tilde{\alpha}\boldsymbol{g}.
      \]
      如果希望两种 batch size 在使用同样多样本后产生相近的参数位移, 就令
      \[
        -\tilde{\alpha}\boldsymbol{g}
        \approx
        -M\alpha\boldsymbol{g},
        \qquad
        \tilde{\alpha}\approx M\alpha.
      \]
      这正是 batch size 扩大 $M$ 倍时学习率也扩大 $M$ 倍的线性缩放规则.
      \item 需要注意, 上面的推导比较的是参数位移, 不是“梯度本身变大了 $M$ 倍”. 在常见实现中, batch 梯度是样本梯度的平均值, batch size 从 $K$ 变为 $MK$ 后, 平均梯度的尺度仍近似为 $\boldsymbol{g}$; 学习率变大的理由是小 batch 在同一个 epoch 内原本会走 $M$ 步, 大 batch 只走 $1$ 步. 这个推导还依赖“短时间内梯度变化不大”、优化器近似为普通 SGD、不同 batch 看到的方向相近等近似, 在训练初期或学习率很大时未必成立. 所以大 batch 训练常配合 learning rate warmup: 先用较小学习率启动, 再逐渐升到目标学习率, 以避免随机初始化阶段的大梯度和大学习率共同造成不稳定.
    \end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{随机初始化与尺度选择}
    \begin{itemize}
      ...
      \]
      真正关键的问题不是“高斯还是均匀”, 而是方差 $\sigma_w^2$ 或区间半径 $r$ 应该取多大. 对称均匀分布的方差为
      \[
        \operatorname{Var}\left(\operatorname{Unif}[-r,r]\right)
        =
        \frac{4r^2}{12}
        =
        \frac{r^2}{3},
      \]
      因而如果希望均匀初始化具有方差 $\sigma_w^2$, 就应取 $r=\sqrt{3}\sigma_w$.
      ...
      \item Xavier 初始化的目的就是根据神经元的输入个数和输出个数来调节参数方差. 先从正向传播看. 对于输入维度为 $n_{\mathrm{in}}$, 输出维度为 $n_{\mathrm{out}}$ 的线性层, 若激活函数先近似看成恒等映射 $f(z)=z$, 一个输出神经元可写为
      \[
        a_j^{\mathrm{out}}
        =
        f\left(\sum_{i=1}^{n_{\mathrm{in}}} w_{ji}a_i^{\mathrm{in}}\right)
        =
        \sum_{i=1}^{n_{\mathrm{in}}} w_{ji}a_i^{\mathrm{in}}.
      \]
      假设 $w_{ji}$ 与 $a_i^{\mathrm{in}}$ 相互独立, 各自同分布, 且期望都为 $0$, 则
      \[
        \operatorname{Var}(a_j^{\mathrm{out}})
        =
        \sum_{i=1}^{n_{\mathrm{in}}}
        \operatorname{Var}(w_{ji}a_i^{\mathrm{in}})
        =
        n_{\mathrm{in}}\operatorname{Var}(W)\operatorname{Var}(a^{\mathrm{in}}).
      \]
      因此若希望正向传播中 $\operatorname{Var}(a^{\mathrm{out}})\approx\operatorname{Var}(a^{\mathrm{in}})$, 就需要
      \[
        \operatorname{Var}(W)\approx\frac{1}{n_{\mathrm{in}}}.
      \]
      从反向传播看, 线性层中某个输入方向的梯度近似为
      \[
        \delta_i^{\mathrm{in}}
        =
        \frac{\partial\mathcal{L}}{\partial a_i^{\mathrm{in}}}
        =
        \sum_{j=1}^{n_{\mathrm{out}}} w_{ji}\delta_j^{\mathrm{out}},
      \]
      在同样的独立、零均值近似下,
      \[
        \operatorname{Var}(\delta_i^{\mathrm{in}})
        =
        n_{\mathrm{out}}\operatorname{Var}(W)\operatorname{Var}(\delta^{\mathrm{out}}).
      \]
      因此若希望反向梯度方差也大致保持不变, 则需要
      \[
        \operatorname{Var}(W)\approx\frac{1}{n_{\mathrm{out}}}.
      \]
      当前向约束和反向约束不能同时精确满足时, Xavier 初始化用 $n_{\mathrm{in}}$ 与 $n_{\mathrm{out}}$ 的平均规模作折中, 即令
      \[
        \frac{n_{\mathrm{in}}+n_{\mathrm{out}}}{2}\operatorname{Var}(W)\approx 1,
        \qquad
        \operatorname{Var}(W) \approx \frac{2}{n_{\mathrm{in}}+n_{\mathrm{out}}}.
      \]
      这样做并不是说某一侧完全正确, 而是在正向激活不爆炸、不消失和反向梯度不爆炸、不消失之间作对称折中, 避免只照顾输入端或只照顾输出端.
      \item 对应的均匀初始化可取
      \[
      ...
      \]
      其中根号下的 $6$ 来自两部分: 均匀分布有 $\operatorname{Var}(\operatorname{Unif}[-r,r])=r^2/3$, Xavier 方差为 $2/(n_{\mathrm{in}}+n_{\mathrm{out}})$, 合起来就是
      \[
        \frac{r^2}{3}
        =
      ...
      \]
      Xavier 初始化主要适合 Tanh 等在零附近近似线性的平滑激活函数. 更一般地, 若非线性在初始化附近可近似为 $f(z)\approx c z$, 则前面的方差推导只会多出一个常数因子 $c^2$, 可以通过额外的增益系数进行修正. Sigmoid 本身不是零中心函数, 但在零附近有 $\sigma(z)-1/2\approx z/4$, 因而也可以用类似的局部线性近似来理解; 只是它的非零均值和饱和区会使深层网络训练更容易出现偏移和梯度变小的问题.
...
\begin{enumerate}
  ...
  \item \textbf{常见输入归一化方法}
    \begin{itemize}
      ...
      \item 白化 (Whitening) 进一步降低特征之间的相关性. PCA 是 Principal Component Analysis, 即主成分分析. 若中心化后的协方差矩阵分解为 $\boldsymbol{\Sigma}=\boldsymbol{U}\boldsymbol{\Lambda}\boldsymbol{U}^{\top}$, 其中 $\boldsymbol{U}$ 的列向量是协方差矩阵的特征向量, $\boldsymbol{\Lambda}$ 的对角元是对应特征值, PCA 白化可粗略写为
      \[
        \tilde{\boldsymbol{x}}
        =
      ...
      \[
        ...
        \boldsymbol{\Lambda},
      \]
      新坐标下的协方差矩阵是对角矩阵, 不同主方向之间的二阶相关性就被消掉了; 第 $i$ 个方向的方差为 $\lambda_i$. 再乘以 $\boldsymbol{\Lambda}^{-1/2}$, 就是在第 $i$ 个主方向上除以标准差 $\sqrt{\lambda_i}$, 因而
      \[
        \operatorname{Cov}(\tilde{\boldsymbol{x}})
        =
        \boldsymbol{\Lambda}^{-\frac{1}{2}}
        \boldsymbol{\Lambda}
        \boldsymbol{\Lambda}^{-\frac{1}{2}}
        ...
        \boldsymbol{I}.
      \]
      这说明在理想估计下, 白化后的各主方向方差都变为 $1$, 方向之间的协方差变为 $0$. 这里说“更接近”和“降低”, 是因为实际数据有限、协方差估计有误差, 实现中还常把 $\boldsymbol{\Lambda}^{-1/2}$ 写成 ${\left(\boldsymbol{\Lambda}+\epsilon\boldsymbol{I}\right)}^{-\frac{1}{2}}$ 来避免小特征值导致数值不稳定. 需要注意, 白化只消除了二阶相关性, 并不保证非高斯特征之间完全独立; 而且它需要估计和分解协方差矩阵, 计算和数值代价都高. 在很多深度学习任务中, 对输入做标准化已经足够.
...
\begin{enumerate}
  ...
  \item \textbf{$\ell_1$ 与 $\ell_2$ 正则化}
    \begin{itemize}
      ...
      \]
      其中 $p$ 常取 $1$ 或 $2$, $\lambda$ 为正则化系数. 若参数向量为 $\boldsymbol{\theta}=(\theta_1,\ldots,\theta_d)$, 则
      \[
        \|\boldsymbol{\theta}\|_1
        =
        \sum_{i=1}^{d}|\theta_i|,
  ...
  \item \textbf{Dropout}
    \begin{itemize}
      ...
      \]
      这里除以 $q$ 是 inverted dropout 的常见实现. 期望是对随机掩码 $\boldsymbol{m}$ 取的; 在计算这一期望时, 通常把当前样本已经得到的激活 $\boldsymbol{h}$ 看成给定值, 并假设掩码 $m_i$ 与 $h_i$ 独立, 且
      \[
        \mathbb{P}(m_i=1)=q,
        \qquad
        \mathbb{P}(m_i=0)=1-q.
      \]
      因此 $\mathbb{E}[m_i]=q$, 逐坐标有
      \[
        \mathbb{E}[\tilde{h}_i\mid h_i]
        =
        \mathbb{E}\left[\frac{m_i h_i}{q}\mid h_i\right]
        =
        \frac{h_i}{q}\mathbb{E}[m_i]
        =
        h_i.
      \]
      合在一起就是 $\mathbb{E}[\tilde{\boldsymbol{h}}\mid\boldsymbol{h}]=\boldsymbol{h}$. 如果训练时不除以 $q$, 则期望会变成 $q\boldsymbol{h}$, 推理阶段就需要再乘以 $q$ 或做相应尺度修正. Inverted dropout 把这个尺度修正放在训练阶段完成, 因而推理阶段关闭 Dropout 后可以直接使用完整激活. 需要注意, 这个结论只保证当前层被丢弃后激活的逐坐标期望不变; 经过后续非线性层后, 整个网络输出的期望不一定严格等于无 Dropout 网络的输出.
      ...
      \]
      更重要的是把它理解为训练阶段的随机正则化, 不必把每次普通推理都解释成真实的贝叶斯积分.
