...
\begin{enumerate}
  \item \textbf{全连接有什么问题}: 说明固定输入输出映射为什么难以处理带顺序、上下文和历史状态的序列问题.
  \item \textbf{循环神经网络简介}: 说明 RNN 如何用隐藏状态保存历史信息, 并在时间维度上复用同一套参数.
  \item \textbf{应用到机器学习}: 说明序列到类别、同步序列标注和异步序列到序列任务如何用 RNN 表达.
  \item \textbf{参数学习与长程依赖问题}: 说明 BPTT 如何计算循环网络梯度, 以及梯度消失和梯度爆炸为什么会限制长程记忆.
  \item \textbf{LSTM、GRU 与其他改进结构}: 说明门控机制如何控制写入、保留、遗忘和输出, 并讨论现代 RNN 与长序列建模.
\end{enumerate}

\textbf{如果细节都忘了}: 最应该记住的是, RNN 的核心是用一个随时间更新的隐藏状态压缩历史信息. 这个状态让模型能处理变长序列和上下文依赖, 但也带来梯度长链路、记忆容量和并行效率问题; LSTM、GRU、RWKV、Mamba 等改进都在围绕这些问题做折中.

\textbf{问题思考}:
\begin{itemize}
  \item 为什么“能处理任意长度序列”和“能无损记住任意长历史”不是一回事?
\end{itemize}
...
\textbf{本节导读}:
\begin{itemize}
  \item \textbf{本节目的}: 说明普通全连接前馈网络为什么不自然适合序列数据.
  \item \textbf{为什么学}: 只有先看清固定映射和固定窗口的局限, 才能理解 RNN 为什么要引入隐藏状态和参数共享.
  \item \textbf{核心内容}: 序列任务中当前输出常依赖历史上下文, 而前馈网络默认只学习固定维度输入到输出的静态映射.
  \item \textbf{最小例子}: 在 ``arrive Taipei'' 和 ``leave Taipei'' 中, 同一个词 ``Taipei'' 的槽位标签不同; 若模型只看当前词向量, 就无法区分它是目的地还是出发地.
  \item \textbf{最该记住}: 序列建模需要内部状态来承接历史, 不能只靠把当前输入送进一个固定函数.
\end{itemize}
...
\textbf{本节导读}:
\begin{itemize}
  \item \textbf{本节目的}: 引入 RNN 的隐藏状态、递推公式、时间展开和参数共享.
  \item \textbf{为什么学}: RNN 是理解序列模型、语言模型、BPTT、长程依赖和门控结构的基础.
  \item \textbf{核心内容}: RNN 在每个时间步用当前输入和上一隐藏状态计算新隐藏状态, 从而把历史压缩进固定维度向量.
  \item \textbf{最小例子}: 判断一个二值序列中当前位置是否形成连续两个 1, 只需隐藏状态保存上一时刻输入, 当前输入再与它共同决定输出.
  \item \textbf{最该记住}: RNN 的记忆不是保存全部历史, 而是用可学习递推把对任务有用的历史压缩进隐藏状态.
\end{itemize}
...
\begin{enumerate}
  \item \textbf{任务目标}
    ...
    输入是一个二值序列
    \begin{equation*}
      X=(x_1,x_2,\ldots,x_T), \quad x_t\in\{0,1\}.
    \end{equation*}
    输出也是一个按时间步给出的二值序列
    \begin{equation*}
      Y=(y_1,y_2,\ldots,y_T), \quad y_t\in\{0,1\},
    \end{equation*}
    其中
    \begin{equation*}
      y_t =
  ...
  \item \textbf{从输出公式反推隐藏状态}
    ...
    \begin{equation*}
      ...
      = \operatorname{ReLU}\left(x_t + x_{t-1} - 1\right),
    \end{equation*}
    那么隐藏状态 $\boldsymbol{h}_t$ 至少要让输出层读到三项信息: 当前输入 $x_t$, 上一时刻输入 $x_{t-1}$, 以及一个常数 1 用来产生 $-1$ 这个偏置. 因此最直接的选择是
    \begin{equation*}
      \boldsymbol{h}_t =
      \begin{bmatrix}
        x_t \\
  ...
  \item \textbf{反推循环更新矩阵}
    ...
    \begin{itemize}
      \item 第 1 行要产生 $x_t$, 因而从输入到隐藏的权重取 1, 从旧状态来的权重全取 0.
      \item 第 2 行要产生 $x_{t-1}$, 它正好是 $\boldsymbol{h}_{t-1}$ 的第 1 个分量, 因而循环矩阵第 2 行取 $[1,0,0]$.
      \item 第 3 行要保持常数 1, 因而循环矩阵第 3 行取 $[0,0,1]$, 并且不接收当前输入.
    \end{itemize}
    于是得到
    \begin{equation*}
      \boldsymbol{W}_{xh} =
    ...
    \begin{equation*}
      ...
      \end{bmatrix}.
    \end{equation*}
    由于这里的三个分量都非负, ReLU 不会改变它们. 因此由归纳可知, 该 RNN 的隐藏状态始终满足 $\boldsymbol{h}_t^\top=(x_t,x_{t-1},1)$.
  ...
  \item \textbf{例子}
    ...
    \begin{equation*}
      Y=(0,0,0,0,1,1,0,0,1).
    \end{equation*}
    这个例子展示了 RNN 的一个关键思想: 隐藏状态不必保存完整历史, 只要保存解决当前任务所需的充分信息即可. 对“连续两个 1”这个任务来说, 当前输入和上一时刻输入就是充分信息; 矩阵 $\boldsymbol{W}_{hh}$ 的作用正是把上一时刻的当前输入向后移动一格, 形成可被输出层读取的一步记忆.
\end{enumerate}
...
\textbf{本节导读}:
\begin{itemize}
  \item \textbf{本节目的}: 说明 RNN 在机器学习任务中的几种常见输入输出模式.
  \item \textbf{为什么学}: 只有明确输入序列和输出序列的时间关系, 才能选择合适的汇聚、标注或解码方式.
  \item \textbf{核心内容}: 序列任务可分为序列到类别、同步序列到序列、异步序列到序列和 Encoder-Decoder 等形式.
  \item \textbf{最小例子}: 情感分类把整句话读完后输出一个标签, 中文分词则要求每个字输出一个位置标签, 机器翻译还需要生成长度可能不同的目标序列.
  \item \textbf{最该记住}: RNN 提供的是状态递推框架, 具体任务还要决定在哪些时间步读入、汇聚和输出.
\end{itemize}
...
\textbf{本节导读}:
\begin{itemize}
  \item \textbf{本节目的}: 说明 RNN 参数如何通过时间展开做反向传播, 以及长程依赖为什么难学.
  \item \textbf{为什么学}: RNN 看似能携带任意长历史, 但训练时梯度要沿时间链路反传, 这会直接限制模型能学到多远的依赖.
  \item \textbf{核心内容}: BPTT 把所有时间步的共享参数梯度累加起来; 反向信号中的 Jacobian 连乘会导致梯度消失或爆炸.
  \item \textbf{最小例子}: 若每向前反传一步梯度都乘以约 $0.5$, 经过 20 步后只剩约 $0.5^{20}$, 早期输入几乎无法影响参数更新.
  \item \textbf{最该记住}: 长程依赖困难不是 RNN “理论上不能看远”, 而是梯度和状态在长时间链路上传递不稳定.
\end{itemize}
...
\item \textbf{梯度消失与梯度爆炸}
  \begin{itemize}
    \item 为了看清楚问题的来源, 先考虑最简单的情形: 损失只定义在最后一个隐藏状态上,
    \begin{equation*}
      \mathcal{L}=\mathcal{L}(\boldsymbol{h}_T),
    \end{equation*}
    RNN 的状态递推写为
    \begin{align*}
      \boldsymbol{z}_t
      &= \boldsymbol{M}_1\boldsymbol{h}_{t-1}
       + \boldsymbol{M}_2\boldsymbol{x}_t
       + \boldsymbol{b}, \\
      \boldsymbol{h}_t &= f(\boldsymbol{z}_t).
    \end{align*}
    其中 $\boldsymbol{M}_1$ 是循环权重, 即上一时刻隐藏状态到当前隐藏状态的连接. 训练时对它做梯度下降:
    \begin{equation*}
      \boldsymbol{M}_1
      \leftarrow
      \boldsymbol{M}_1
      -\eta
      \frac{\partial \mathcal{L}}{\partial \boldsymbol{M}_1}.
    \end{equation*}
    因此关键问题变成: 最后一时刻的损失如何一路反传到各个时间步上对同一个 $\boldsymbol{M}_1$ 的使用.

    \item 从最后一步开始. 记
    \begin{equation*}
      \boldsymbol{g}_T
      =
      \frac{\partial \mathcal{L}}{\partial \boldsymbol{h}_T},
      \quad
      \boldsymbol{\delta}_T
      =
      \frac{\partial \mathcal{L}}{\partial \boldsymbol{z}_T}
      =
      \boldsymbol{g}_T \odot f'(\boldsymbol{z}_T).
    \end{equation*}
    因为 $\boldsymbol{z}_T=\boldsymbol{M}_1\boldsymbol{h}_{T-1}+\boldsymbol{M}_2\boldsymbol{x}_T+\boldsymbol{b}$, 所以最后一步对循环权重的梯度贡献是
    \begin{equation*}
      \left.\frac{\partial \mathcal{L}}{\partial \boldsymbol{M}_1}\right|_{T}
      =
      \boldsymbol{\delta}_T\boldsymbol{h}_{T-1}^{\top}.
    \end{equation*}
    这仍然是前馈网络中熟悉的形式: 误差信号乘以上一层输入. 区别在于, 这里的“上一层输入”是上一时刻的隐藏状态.

    \item 再往前退一步看 $T-1$. 损失 $\mathcal{L}$ 不直接依赖 $\boldsymbol{h}_{T-1}$, 但它通过
    $\boldsymbol{h}_{T-1}\to\boldsymbol{z}_T\to\boldsymbol{h}_T\to\mathcal{L}$ 间接依赖它. 因此
    \begin{equation*}
      \frac{\partial \mathcal{L}}{\partial \boldsymbol{h}_{T-1}}
      =
      \boldsymbol{M}_1^{\top}\boldsymbol{\delta}_T.
    \end{equation*}
    再乘上本时刻激活函数的导数, 得到
    \begin{equation*}
      \boldsymbol{\delta}_{T-1}
      =
      \frac{\partial \mathcal{L}}{\partial \boldsymbol{z}_{T-1}}
      =
      \left(\boldsymbol{M}_1^{\top}\boldsymbol{\delta}_T\right)
      \odot f'(\boldsymbol{z}_{T-1}).
    \end{equation*}
    因而 $T-1$ 时刻对同一个 $\boldsymbol{M}_1$ 的梯度贡献为
    \begin{equation*}
      \left.\frac{\partial \mathcal{L}}{\partial \boldsymbol{M}_1}\right|_{T-1}
      =
      \boldsymbol{\delta}_{T-1}\boldsymbol{h}_{T-2}^{\top}.
    \end{equation*}

    \item 继续往前退到 $T-2$, 同样有
    \begin{align*}
      \boldsymbol{\delta}_{T-2}
      &=
      \left(\boldsymbol{M}_1^{\top}\boldsymbol{\delta}_{T-1}\right)
      \odot f'(\boldsymbol{z}_{T-2}) \\
      &=
      \operatorname{diag}\left(f'(\boldsymbol{z}_{T-2})\right)
      \boldsymbol{M}_1^{\top}
      \operatorname{diag}\left(f'(\boldsymbol{z}_{T-1})\right)
      \boldsymbol{M}_1^{\top}\boldsymbol{\delta}_T.
    \end{align*}
    这一步已经显示出长程依赖的核心结构: 每向前多传一个时间步, 反向信号就要多乘一次 $\boldsymbol{M}_1^{\top}$ 和一次激活函数导数.
    ...
    \end{equation*}
    因为同一个循环权重 $\boldsymbol{M}_1$ 在所有时间步共享, 所以总梯度要把各时刻的贡献加起来:
    \begin{equation*}
      \frac{\partial \mathcal{L}}{\partial \boldsymbol{M}_1}
      =
      \sum_{k=1}^{T}
      \boldsymbol{\delta}_k\boldsymbol{h}_{k-1}^{\top}.
    \end{equation*}
    展开其中较早时间步的误差信号, 可以看到
    \begin{equation*}
      \boldsymbol{\delta}_k
      =
      \operatorname{diag}\left(f'(\boldsymbol{z}_k)\right)\boldsymbol{M}_1^{\top}
      \operatorname{diag}\left(f'(\boldsymbol{z}_{k+1})\right)\boldsymbol{M}_1^{\top}
      \cdots
      \operatorname{diag}\left(f'(\boldsymbol{z}_{T-1})\right)\boldsymbol{M}_1^{\top}
      \boldsymbol{\delta}_T.
    \end{equation*}
    因而从最终损失反传到很早的时间步时, 梯度不可避免地包含一长串 Jacobian 矩阵连乘.

    \item 梯度消失和梯度爆炸正是由这串连乘造成的. 若粗略记
    \begin{equation*}
      \gamma_\tau
      =
      \left\|
      \operatorname{diag}\left(f'(\boldsymbol{z}_\tau)\right)
      \boldsymbol{M}_1^{\top}
      \right\|,
    \end{equation*}
    则有近似关系
    \begin{equation*}
      \|\boldsymbol{\delta}_k\|
      \lesssim
      \left(\prod_{\tau=k}^{T-1}\gamma_\tau\right)
      \|\boldsymbol{\delta}_T\|.
    \end{equation*}
    ...
    \item 这里的梯度消失并不是说总梯度 $\partial\mathcal{L}/\partial \boldsymbol{M}_1$ 一定变成零. 总梯度是许多时间步贡献的和, 靠近 $T$ 的项仍可能很大. 真正的问题是较早输入经过长链路传来的贡献被压得很小, 于是参数更新主要由短期依赖决定, 模型难以学习“很久以前的信息影响当前输出”这类规律.
    \item 梯度爆炸则会造成 $\boldsymbol{M}_1$ 的更新过大, 使损失震荡甚至出现数值溢出. 常用工程处理包括梯度截断
    \begin{equation*}
      \boldsymbol{g}
      \leftarrow
...
\textbf{问题思考}:
\begin{itemize}
  \item 为什么 BPTT 中共享参数的梯度需要累加所有时间步的贡献?
  \item 梯度消失时, 模型最先失去的是短期依赖还是长程依赖? 为什么?
  \item 梯度截断能缓解梯度爆炸, 为什么不能直接解决梯度消失?
\end{itemize}
...
\textbf{本节导读}:
\begin{itemize}
  \item \textbf{本节目的}: 说明门控循环网络如何缓解简单 RNN 的长程依赖和记忆控制问题.
  \item \textbf{为什么学}: 简单 RNN 的状态会不断被重写, 梯度也不稳定; 门控机制提供了可学习的信息保留、写入和遗忘方式.
  \item \textbf{核心内容}: GRU 用更新门和重置门控制隐藏状态, LSTM 用输入门、遗忘门、输出门和细胞状态区分存储与输出, 现代 RNN 则进一步追求长序列效率.
  \item \textbf{最小例子}: 若一句话开头出现否定词, LSTM 的遗忘门可以让相关信息在细胞状态中保留多个时间步, 直到后面情感判断需要它.
  \item \textbf{最该记住}: 门控的核心不是“记得越多越好”, 而是让模型学会什么该写入、什么该保留、什么该丢掉.
\end{itemize}
