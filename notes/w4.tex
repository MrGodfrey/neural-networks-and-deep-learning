...
除本导读外, 本讲共有 6 个主体节. 它们从人工神经元和前馈网络开始, 讲清楚为什么需要非线性激活, 如何用反向传播和自动微分训练网络, 以及深层网络优化为什么会变难.

\begin{enumerate}
    \item \textbf{神经网络概览}: 说明人工神经元、网络连接结构以及连接主义思想的基本含义.
    \item \textbf{前馈神经网络}: 说明多层感知器如何按层进行前向传播, 以及非线性组合为什么能增强表达能力.
    \item \textbf{激活函数}: 说明 Sigmoid、Tanh、ReLU、Swish、GELU 等激活函数如何影响表达能力和梯度传播.
    \item \textbf{反向传播算法}: 说明如何利用链式法则逐层计算偏置、权重和误差项的梯度.
    \item \textbf{计算图与自动微分}: 说明深度学习框架如何把复杂函数拆成计算图并自动执行反向求导.
    \item \textbf{优化问题}: 说明神经网络训练为什么通常是非凸优化, 以及梯度消失等问题为什么会影响深层网络训练.
\end{enumerate}

\textbf{如果细节都忘了}: 最应该记住的是, 神经网络就是反复做“线性变换 + 非线性激活”的可训练函数组合; 训练它的关键是用反向传播高效计算梯度, 但深层结构会带来非凸优化、梯度消失和训练稳定性问题.

\textbf{问题思考}:
\begin{itemize}
    \item 为什么只有线性层堆叠不能真正增强神经网络的表达能力?
\end{itemize}
...
\textbf{本节导读}:
\begin{itemize}
    \item \textbf{本节目的}: 从人工神经元和网络连接方式出发, 建立神经网络的基本直觉.
    \item \textbf{为什么学}: 后续前馈网络、激活函数和反向传播都建立在“神经元如何计算、网络如何连接”这个基础上.
    \item \textbf{核心内容}: 人工神经元先做加权求和, 再通过激活函数产生输出; 神经网络则把大量神经元连接起来学习复杂映射.
    \item \textbf{最小例子}: 单个人工神经元计算 $z=w^\top x+b$, 再用阶跃函数输出类别, 就得到感知器的基本形式.
    \item \textbf{最该记住}: 神经网络的知识主要体现在连接权重中, 学习过程就是不断调整这些连接强度.
\end{itemize}
...
\textbf{本节导读}:
\begin{itemize}
    \item \textbf{本节目的}: 说明前馈神经网络如何按层组织信息流, 以及多层结构如何形成复合函数.
    \item \textbf{为什么学}: 前馈网络是后续卷积网络、循环网络和深度模型的基本骨架, 也是理解反向传播的直接对象.
    \item \textbf{核心内容}: 每一层先做仿射变换, 再做非线性激活; 多层复合后, 网络可以表示比单层线性模型复杂得多的函数.
    \item \textbf{最小例子}: 两层网络先把输入 $\boldsymbol{x}$ 映射为隐藏表示 $\boldsymbol{a}^{(1)}$, 再把隐藏表示映射为输出 $\hat{\boldsymbol{y}}$, 中间的非线性激活使它不再只是一个线性模型.
    \item \textbf{最该记住}: 前馈网络的核心结构是层间前向传播, 表达能力来自多层线性变换与非线性激活的组合.
\end{itemize}
...
\textbf{本节导读}:
\begin{itemize}
    \item \textbf{本节目的}: 说明激活函数为什么必须引入非线性, 以及不同激活函数如何影响训练.
    \item \textbf{为什么学}: 激活函数既决定网络能表达什么样的非线性关系, 也直接影响梯度是否容易传播.
    \item \textbf{核心内容}: Sigmoid 和 Tanh 平滑但容易饱和, ReLU 简单高效但可能死亡, Swish 和 GELU 则通过平滑门控改善表示与优化.
    \item \textbf{最小例子}: 如果所有层都不用激活函数, 两层线性变换 $W_2(W_1x)$ 仍可合并成一个线性变换 $(W_2W_1)x$, 深度不会带来新的非线性表达能力.
    \item \textbf{最该记住}: 激活函数不是装饰, 它同时决定网络的非线性表达能力和梯度传播性质.
\end{itemize}
...
\textbf{本节导读}:
\begin{itemize}
    \item \textbf{本节目的}: 推导神经网络参数梯度如何逐层计算, 说明反向传播为什么高效.
    \item \textbf{为什么学}: 没有反向传播, 多层网络的参数学习就只能低效地逐个求导, 难以训练大规模模型.
    \item \textbf{核心内容}: 反向传播把链式法则组织成误差项递推, 每层权重梯度可写为当前层误差项与上一层激活值的外积.
    \item \textbf{最小例子}: 对第 $l$ 层, 若误差项为 $\delta^{(l)}$, 上一层激活为 $\boldsymbol{a}^{(l-1)}$, 则权重梯度为 $\delta^{(l)}(\boldsymbol{a}^{(l-1)})^\top$.
    \item \textbf{最该记住}: 反向传播不是新的微积分规则, 而是链式法则在多层计算图上的高效组织方式.
\end{itemize}
...
\begin{equation*}
    \frac{\partial L}{\partial \boldsymbol{b}^{(l)}} = \frac{\partial \boldsymbol{z}^{(l)}}{\partial \boldsymbol{b}^{(l)}} \frac{\partial L}{\partial \boldsymbol{z}^{(l)}} = I_{M_l}\delta^{(l)} = \delta^{(l)}.
\end{equation*}
也就是说, 偏置项的梯度就是该层的误差项本身.



\subsection{权重矩阵 $\boldsymbol{W}^{(l)}$ 的梯度}


根据链式法则, 有
\begin{align*}
    \frac{\partial L}{ \partial \boldsymbol{W}^{(l)}_{i j}}
    =
    \frac{\partial \boldsymbol{z}^{(l)}}{\partial \boldsymbol{W}^{(l)}_{i j}} \frac{\partial L}{\partial \boldsymbol{z}^{(l)}}
    =
    \frac{\partial \boldsymbol{z}^{(l)}}{\partial \boldsymbol{W}^{(l)}_{i j}}  \delta^{(l)}
    .
\end{align*}
由于
\begin{align*}
     \frac{\partial \boldsymbol{z}^{(l)}}{\partial \boldsymbol{W}^{(l)}_{i j}}
     & =
     \bigg( \frac{\partial z_k^{(l)}}{\partial W_{ij}^{(l)}} \bigg)_{k}
     \\
    & =
    \bigg( \frac{\partial (\sum_{m} W_{km}^{(l)} a_m^{(l-1)} + b_k^{(l)})}{\partial W_{ij}^{(l)}} \bigg)_{k}
    \\
    & =
    \bigg( a_j^{(l-1)} \delta_{ik} \bigg)_{k}
    ,
\end{align*}
其中 $\delta_{ik}$ 是 Kronecker delta, 因此
\begin{equation*}
    \frac{\partial L}{\partial \boldsymbol{W}^{(l)}_{i j}} = a_j^{(l-1)} \delta_i^{(l)}.
\end{equation*}
即
\begin{align*}
    \frac{\partial L}{\partial \boldsymbol{W}^{(l)}}
    =
    \delta^{(l)} (\boldsymbol{a}^{(l-1)})^\top
    .
\end{align*}

这说明第 $l$ 层参数更新所需的信息非常直接: 一部分来自上一层的激活值 $\boldsymbol{a}^{(l-1)}$, 另一部分来自当前层的误差项 $\delta^{(l)}$. 训练时只要从输出层开始逐层向前计算这些误差项, 就能得到各层参数的梯度, 这正是反向传播算法的核心思想.

\subsection{误差项的递推公式}

当前层误差项可以由下一层误差项反向递推得到:
\begin{equation*}
    \delta^{(l)} = \frac{\partial L}{\partial \boldsymbol{z}^{(l)}} = \frac{\partial \boldsymbol{a}^{(l)}}{\partial \boldsymbol{z}^{(l)}} \frac{\partial \boldsymbol{z}^{(l+1)}}{\partial \boldsymbol{a}^{(l)}} \delta^{(l+1)}.
\end{equation*}
其中, 激活函数对输入的导数矩阵为
\begin{equation*}
...
于是得到反向传播的核心递推关系
\begin{equation*}
    \delta^{(l)} = f_l'\left(\boldsymbol{z}^{(l)}\right) \odot \left( \left(\boldsymbol{W}^{(l+1)}\right)^\top \delta^{(l+1)} \right).
\end{equation*}
...
\textbf{本节导读}:
\begin{itemize}
    \item \textbf{本节目的}: 说明自动微分如何用计算图表示复合函数, 并自动计算梯度.
    \item \textbf{为什么学}: 现代深度学习框架依赖自动微分训练模型, 理解计算图有助于解释梯度累积、共享参数和动态图调试.
    \item \textbf{核心内容}: 计算图把复杂表达式拆成基本运算节点, 反向模式自动微分从输出标量反向累积各变量梯度.
    \item \textbf{最小例子}: 若 $f={(wx+b)}^2$, 计算图会先计算乘法、加法和平方, 反向传播时再把梯度沿这些节点依次传回 $w$、$x$ 和 $b$.
    \item \textbf{最该记住}: 神经网络框架中的自动求导本质上就是在计算图上系统执行链式法则.
\end{itemize}
...
\textbf{本节导读}:
\begin{itemize}
    \item \textbf{本节目的}: 说明神经网络训练目标为什么通常难以优化, 以及梯度传播问题从哪里来.
    \item \textbf{为什么学}: 能写出网络和梯度并不等于训练一定成功, 优化地形和梯度尺度会直接影响模型能否学到有效参数.
    \item \textbf{核心内容}: 多层非线性网络带来非凸目标, 反向传播中的导数连乘会造成梯度消失或爆炸, 因而训练需要合适的初始化、激活函数和优化策略.
    \item \textbf{最小例子}: 即使只有 $y=\sigma(w_2\sigma(w_1x))$ 这样的两层标量网络, 输出关于 $w_1,w_2$ 也已经是嵌套非线性函数, 训练目标通常不再是凸函数.
    \item \textbf{最该记住}: 深度网络训练的难点不只是参数多, 更在于非凸优化和长链路梯度传播不稳定.
\end{itemize}
