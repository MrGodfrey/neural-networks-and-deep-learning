...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item 一般是将所有的向量排成列向量（例如将目标值排成列向量 $\boldsymbol{y}$，特征排成矩阵 $\boldsymbol{X}$）。
            \item 然后看如何进行维数匹配，将求和转换为矩阵乘积。
        \end{itemize}
        通过上述技巧，经验风险可以等价地表示为：
        \begin{equation*}
        ...
        \item \textbf{补充：矩阵微积分基础}。为了求解使得经验风险最小化的最优参数 $\boldsymbol{w}^*$，我们需要用到矩阵微积分知识（无需详细推导，掌握公式即可）：
        \begin{itemize}
            \item \textbf{标量关于向量的偏导}：例如 $\frac{\partial (\boldsymbol{a}^{\top}\boldsymbol{x})}{\partial \boldsymbol{x}} = \boldsymbol{a}$
            \item \textbf{向量关于向量的偏导}：例如 $\frac{\partial (\boldsymbol{A}\boldsymbol{x})}{\partial \boldsymbol{x}} = \boldsymbol{A}^{\top}$
            \item \textbf{向量函数及其导数与链式法则}：与标量函数的求导法则类似，但需严格注意矩阵乘法的维数匹配。
        \end{itemize}
