...
除本导读外, 本讲共有 4 个主体节. 它们从概率语言开始, 逐步建立线性回归模型, 推导参数估计方法, 再用概率视角解释为什么平方损失和正则化会自然出现.

\begin{enumerate}
  \item \textbf{概率论基础回顾}: 回顾概率空间、随机变量、联合分布、条件概率、采样、期望和大数定律, 为后续概率建模打基础.
  \item \textbf{线性回归概述}: 说明线性回归如何用特征的线性组合预测连续值, 以及它为什么是重要的基线模型.
  \item \textbf{参数估计方法}: 说明如何通过平方损失最小化学习参数, 以及正则化如何改善病态问题和过拟合.
  \item \textbf{概率视角解释}: 说明高斯噪声、最大似然估计、最大后验估计与线性回归正则化之间的关系.
\end{enumerate}

\textbf{如果细节都忘了}: 最应该记住的是, 线性回归既可以看作最小化平方误差的函数拟合问题, 也可以看作高斯噪声假设下的概率估计问题; 加入 $L_2$ 正则化, 就相当于给参数加入“不要太大”的先验偏好.

\textbf{问题思考}:

\begin{itemize}
  \item 为什么同一个线性回归公式, 可以同时从几何误差和概率生成两个角度理解?
\end{itemize}
...
\textbf{本节导读}:
\begin{itemize}
  \item \textbf{本节目的}: 回顾描述随机性所需的基本概念, 为后面用概率解释模型和损失函数做准备.
  \item \textbf{为什么学}: 机器学习中的数据、噪声、采样误差和泛化能力都离不开概率语言, 不先统一记号会影响后续推导.
  \item \textbf{核心内容}: 概率空间定义随机试验, 随机变量把结果映射为数值, 分布、条件概率、期望和大数定律则描述随机变量的规律.
  \item \textbf{最小例子}: 掷一枚骰子时, 样本空间是 $\{1,\ldots,6\}$, 随机变量可以取点数本身, 期望表示长期平均点数, 大数定律说明样本平均会逐渐接近期望.
  \item \textbf{最该记住}: 概率论给机器学习提供了处理不确定性和用样本近似总体的语言.
\end{itemize}
...
\textbf{本节导读}:
\begin{itemize}
  \item \textbf{本节目的}: 引入线性回归模型, 说明如何用特征的线性组合预测连续输出.
  \item \textbf{为什么学}: 线性回归形式简单、可解释、可推导, 是理解更复杂监督学习模型的基础参照.
  \item \textbf{核心内容}: 模型假设 $\hat{y}=\boldsymbol{w}^{\top}\boldsymbol{x}$, 参数 $\boldsymbol{w}$ 决定各特征对预测值的贡献.
  \item \textbf{最小例子}: 若只用房屋面积预测房价, 可以写成 $\hat{y}=w_0+w_1x$, 其中 $w_1$ 表示面积每增加一个单位时预测价格的变化量.
  \item \textbf{最该记住}: 线性回归的强处在于简单清晰, 局限在于它默认输出与特征之间主要满足线性关系.
\end{itemize}
...
\begin{itemize}
  \item 线性回归为什么适合作为连续值预测任务的基线模型?
  \item 若真实关系明显非线性, 可以通过哪些方式扩展线性回归的表达能力?
\end{itemize}
...
\textbf{本节导读}:
\begin{itemize}
  \item \textbf{本节目的}: 推导如何从训练数据中估计线性回归参数, 并说明正则化为什么能让解更稳定.
  \item \textbf{为什么学}: 模型形式只规定了可选函数, 参数估计才决定最终使用哪一个函数做预测.
  \item \textbf{核心内容}: 平方损失下的经验风险最小化可导出正规方程; 当矩阵病态或模型容易过拟合时, 可加入 $L_2$ 正则项得到岭回归形式.
  \item \textbf{最小例子}: 若两个输入特征几乎完全相同, 最小二乘解可能在两列之间任意分配很大的正负权重; 加入 $\lambda\|\boldsymbol{w}\|^2$ 后, 解会更倾向于较小且稳定的参数.
  \item \textbf{最该记住}: 参数估计就是选择让训练误差小且足够稳定的 $\boldsymbol{w}$, 正则化是在“拟合数据”和“控制参数规模”之间做折中.
\end{itemize}
...
\textbf{本节导读}:
\begin{itemize}
  \item \textbf{本节目的}: 从概率生成过程解释线性回归, 把平方损失、最大似然和正则化联系起来.
  \item \textbf{为什么学}: 同一个训练目标若能从概率角度解释, 就能更清楚地理解噪声假设、参数先验和不确定性.
  \item \textbf{核心内容}: 高斯噪声假设下最大似然等价于最小二乘, 高斯先验下最大后验等价于带 $L_2$ 正则的线性回归.
  \item \textbf{最小例子}: 若温度计读数等于真实线性趋势加上高斯测量误差, 那么让所有残差平方和最小, 就是在寻找最可能生成这些读数的线性模型.
  \item \textbf{最该记住}: 平方损失不是凭空选择的, 它对应高斯噪声; $L_2$ 正则也不是纯技巧, 它对应参数服从零均值高斯先验.
\end{itemize}
...
\begin{itemize}
  \item 为什么高斯噪声假设会自然导出平方损失?
  \item 最大似然估计和最大后验估计的根本区别是什么?
  \item 为什么 $L_2$ 正则化可以解释为对参数加入零均值高斯先验?
\end{itemize}
